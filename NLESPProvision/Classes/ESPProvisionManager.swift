...
public class ESPProvisionManager: NSObject, AVCaptureMetadataOutputObjectsDelegate {
    ...
    public func searchESPDevices(devicePrefix: [String], uuid: String? = nil, mac: String? = nil, transport: ESPTransport, security: ESPSecurity = .secure, completionHandler: @escaping ([ESPDevice]?, ESPDeviceCSSError?) -> Void) {
        ...
        switch transport {
        case .ble:
            espBleTransport = ESPBleTransport(scanTimeout: 11.0, deviceNamePrefix: devicePrefix, uuid: uuid, mac: mac)
    ...
    public func stopESPDevicesSearchDoNothing() {
        ESPLog.log("Stop ESPDevices search called.")
        if espBleTransport == nil {
            return
        }
        espBleTransport.stopSearchDoNothing()
    }
    ...
    public func scanQRCode(scanView: UIView, completionHandler: @escaping (ESPDevice?, ESPDeviceCSSError?) -> Void, scanStatus: ((ESPScanStatus) -> Void)? = nil) {
    ...
    public func refreshDeviceList(completionHandler: @escaping ([ESPDevice]?, ESPDeviceCSSError?) -> Void) {
        searchESPDevices(devicePrefix: devicePrefix, transport: transport, security: security, completionHandler: completionHandler)
    }
    ...
    public func createESPDevice(deviceName: String, transport: ESPTransport, security: ESPSecurity = .secure2, proofOfPossession: String? = nil, softAPPassword: String? = nil, username: String? = nil, completionHandler: @escaping (ESPDevice?, ESPDeviceCSSError?) -> Void) {
        ...
        case .ble:
            ...
            self.security = security
            scanStatusBlock?(.searchingBLE(deviceName))
            espBleTransport = ESPBleTransport(scanTimeout: 5.0, deviceNamePrefix: [deviceName], uuid: "", mac: "", proofOfPossession: proofOfPossession, username: username)
            espBleTransport.scan(delegate: self)
        default:
            scanStatusBlock?(.joiningSoftAP(deviceName))
...
extension ESPProvisionManager: ESPBLETransportDelegate {
    func peripheralsFound(peripherals: [String: ESPDevice]) {
        ...
        searchCompletionHandler?(espDevices, nil)
        scanCompletionHandler?(espDevices.first, nil)
    }

    func peripheralsNotFound(serviceUUID _: UUID?) {
        ESPLog.log("No ble devices found.")

        searchCompletionHandler?(nil, .espDeviceNotFound)
        scanCompletionHandler?(nil, .espDeviceNotFound)
    }
}
