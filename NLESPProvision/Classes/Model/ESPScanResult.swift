...
struct ESPScanResult: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        security = try ESPSecurity(rawValue: container.decodeIfPresent(Int.self, forKey: .security) ?? 2)
        if security == .secure2 {
            username = try container.decodeIfPresent(String.self, forKey: .username)
        }
        if let transportValue = try ESPTransport(rawValue: container.decode(String.self, forKey: .transport)) {
