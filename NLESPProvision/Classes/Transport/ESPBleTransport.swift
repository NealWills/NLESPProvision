...
class ESPBleTransport: NSObject, ESPCommunicable {
    ...
    var characterList: [ESPDevice.CharacteristicStyle: CBCharacteristic] = .init()
    ...
    var bleStatusChangeBlock: ((ESPSessionStatus) -> Void)?

    public var delegate: ESPBLETransportDelegate?
    var currentWriteCharacteristic: CBCharacteristic?
    ...
    init(scanTimeout: TimeInterval, deviceNamePrefix: [String], uuid: String?, mac: String?, proofOfPossession: String? = nil, username: String? = nil) {
    ...
    func SendSessionData(data: Data, sessionPath _: String?, completionHandler: @escaping (Data?, Error?) -> Void) {
        ESPLog.log("Sending session data.")
        guard peripheralCanWrite, peripheralCanRead,
              let espressifPeripheral = currentPeripheral
        else {
    ...
    func connect(peripheral: CBPeripheral, withOptions options: [String: Any]?, delegate: ESPBLEStatusDelegate) {
        ESPDeviceManager.share.map[peripheral] = centralManager
    ...
    func disconnect() {
        if let currentPeripheral = currentPeripheral {
            ESPLog.info("Cancelling peripheral connection.")
            centralManager.cancelPeripheralConnection(currentPeripheral)
        }
    }
...
extension ESPBleTransport: CBCentralManagerDelegate {
    func centralManagerDidUpdateState(_ central: CBCentralManager) {
        ...
        case .unknown:
            ESPLog.info("Bluetooth state unknown")
        case .resetting:
            ESPLog.info("Bluetooth state resetting")
        case .unsupported:
            ESPLog.info("Bluetooth state unsupported")
        case .unauthorized:
            ESPLog.info("Bluetooth state unauthorized")
        case .poweredOff:
            ESPLog.info("Bluetooth state off")
        case .poweredOn:
            ESPLog.info("Bluetooth state on")
            isBLEEnabled = true
            bleScanTimer = Timer.scheduledTimer(timeInterval: scanTimeout,
                                                target: self,
                                                selector: #selector(stopScan(timer:)),
    ...
    func centralManager(_: CBCentralManager,
                        ...
                        rssi: NSNumber) {
        ESPLog.info("Peripheral devices discovered.\(data.debugDescription)")
        if let peripheralName = data["kCBAdvDataLocalName"] as? String ?? peripheral.name {
            ...
            newEspDevice.rssi = Int(rssi)

            if let macData = data["kCBAdvDataManufacturerData"] as? Data {
                let macList = macData.map({
                    return String.init(format: "%02X", $0)
                })
                ...
                newEspDevice.mac = mac.uppercased()
                newEspDevice.macExtra = macExtra?.uppercased()
            }

            if deviceMac.count > 0 {
//                print(peripheral.identifier.uuidString.uppercased(), newEspDevice.mac, deviceMac)
                if newEspDevice.mac.uppercased() == deviceMac.uppercased() {
                    espressifPeripherals[peripheralName] = newEspDevice
                    newEspDevice.peripheral = peripheral
                    delegate?.peripheralsFound(peripherals: espressifPeripherals)
                    stopSearchDoNothing()
                }
            } else if deviceUUID.count > 0 {
                if peripheral.identifier.uuidString.uppercased() == deviceUUID.uppercased() {
                    espressifPeripherals[peripheralName] = newEspDevice
                    newEspDevice.peripheral = peripheral
                    delegate?.peripheralsFound(peripherals: espressifPeripherals)
                    stopSearchDoNothing()
                ...
                if deviceNamePrefix.first(where: { peripheralName.lowercased().contains($0.lowercased()) }) != nil {
                    espressifPeripherals[peripheralName] = newEspDevice
                    newEspDevice.peripheral = peripheral
                    delegate?.peripheralsFound(peripherals: espressifPeripherals)
                }
    ...
    func centralManager(_: CBCentralManager, didConnect device: CBPeripheral) {
        ESPLog.info("Connected to peripheral. Discover services. \(device.description)")
        currentPeripheral?.discoverServices(nil)
        bleStatusChangeBlock?(.connected)
    }

    func centralManager(_: CBCentralManager, didFailToConnect peripheral: CBPeripheral, error: Error?) {
        ESPLog.info("Fail to connect to peripheral. error: \(error?.localizedDescription ?? "Known error")")
        bleStatusDelegate?.peripheralFailedToConnect(peripheral: peripheral, error: error)
        bleStatusChangeBlock?(.failedToConnect(.sessionInitError))
    }

    func centralManager(_: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: Error?) {
        ESPLog.info("Disconnected with peripheral. error: \(error?.localizedDescription ?? "Known error")")
        bleStatusDelegate?.peripheralDisconnected(peripheral: peripheral, error: error)
        bleStatusChangeBlock?(.disconnected)
    }
}
...
//        ESPLog.info("Peripheral did discover services.")
...
//        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error _: Error?) {
        ESPLog.info("Peripheral did discover chatacteristics.")

        service.characteristics?.forEach {
            let characterStyle = ESPDevice.CharacteristicStyle(string: $0.uuid.uuidString)
            switch characterStyle {
            case .MeatmeetProtocol:
                peripheral.setNotifyValue(true, for: $0)
                self.characterList[characterStyle] = $0
                bleConnectTimer.invalidate()
            ...
            case let .Other(string):
                ...
                currentPeripheral?.discoverDescriptors(for: $0)
            }
        }
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        let characterStyle = ESPDevice.CharacteristicStyle(string: characteristic.uuid.uuidString)
        switch characterStyle {
        case .SiliconOTADataAttribute:
            var userInfo: [String: Any] = .init()
            userInfo["uuid"] = peripheral.identifier.uuidString.uppercased()
            userInfo["characteristic"] = characterStyle.uuid
            var array: [UInt8] = .init()
            if let data = characteristic.value {
                array = [UInt8](data)
            }
            userInfo["data"] = array
            let noticeName = Notification.Name("com.esp.siliconData.didUpdateValueFor")
            NotificationCenter.default.post(name: noticeName, object: nil, userInfo: userInfo)
        case .SiliconOTAControlAttribute:
            // 如果是silicon的特征,则走蓝牙OTA代理
            var userInfo: [String: Any] = .init()
            userInfo["uuid"] = peripheral.identifier.uuidString.uppercased()
            userInfo["characteristic"] = characterStyle.uuid
            var array: [UInt8] = .init()
            ...
            userInfo["data"] = array
            let noticeName = Notification.Name("com.esp.siliconController.didUpdateValueFor")
            NotificationCenter.default.post(name: noticeName, object: nil, userInfo: userInfo)
        case .MeatmeetProtocol:
            break
        case let .Other(string):
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        case .MeatmeetProtocol:
            ...
            userInfo["data"] = array
            let noticeName = Notification.Name("com.esp.espProvision.didUpdateValueFor")
            NotificationCenter.default.post(name: noticeName, object: nil, userInfo: userInfo)
        case .SiliconOTAControlAttribute:
            // 如果是silicon的特征,则走蓝牙OTA代理
            var userInfo: [String: Any] = .init()
...
//        print("===========", utility.configUUIDMap.keys)
//        if utility.configUUIDMap.keys.contains("")
        if utility.configUUIDMap.keys.count > 5 && utility.isContainVersionInfo() {
            bleConnectTimer.invalidate()
            bleStatusDelegate?.peripheralConnected()
        }
//        readCounter -= 1
...
//        }
    ...
    func peripheral(_ peripheral: CBPeripheral, didOpen channel: CBL2CAPChannel?, error: (any Error)?) {
        ESPLog.log("Did open channel psm: \(channel?.psm)")
    }

    func peripheralIsReady(toSendWriteWithoutResponse peripheral: CBPeripheral) {
        if let character = currentWriteCharacteristic {
            currentWriteCharacteristic = nil
            self.peripheral(peripheral, didWriteValueFor: character, error: nil)
        }
    }
}
