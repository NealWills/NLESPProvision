...
class ESPUtility {
    ...
    func processDescriptor(descriptor: CBDescriptor) {
        ...
        } else if let associatedCharacteristic = descriptor.characteristic {
            configUUIDMap.updateValue(associatedCharacteristic, forKey: associatedCharacteristic.uuid.uuidString)
        }
    }

    func isContainVersionInfo() -> Bool {
        return configUUIDMap.keys.contains(versionPath)
    }
}
